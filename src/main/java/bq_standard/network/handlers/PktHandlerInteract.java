...
public class PktHandlerInteract implements IPacketHandler
{
    ...
    public void handleServer(NBTTagCompound tag, EntityPlayerMP sender)
    {
		...
		for(DBEntry<IQuest> entry : QuestingAPI.getAPI(ApiReference.QUEST_DB).bulkLookup(qc.getActiveQuests()))
		{
            ...
            {
                if(task.getValue() instanceof TaskInteractItem) ((TaskInteractItem)task.getValue()).onInteract(entry, sender, hand, null, Blocks.AIR.getDefaultState(), sender.getPosition(), isHit);
            }
		}
    }
