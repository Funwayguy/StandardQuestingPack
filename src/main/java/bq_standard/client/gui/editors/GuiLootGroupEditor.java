...
public class GuiLootGroupEditor extends GuiQuesting implements IVolatileScreen
{
	...
	public void initGui()
	{
		...
		lineTitle = new GuiTextField(mc.fontRenderer, guiLeft + sizeX/2 + 9, height/2 - 59, btnWidth - 18, 18);
	...
	public void drawScreen(int mx, int my, float partialTick)
	{
		...
		if(selected != null)
		{
			mc.fontRenderer.drawString("" + new DecimalFormat("0.##").format((float)selected.weight/(float)LootRegistry.getTotalWeight() * 100F) + "%", guiLeft + 16 + (sizeX - 32)/4*3 + 8, guiTop + sizeY/2 - 14, ThemeRegistry.curTheme().textColor().getRGB(), false);
		}
