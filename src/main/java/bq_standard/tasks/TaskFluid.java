	...
	public void detect(EntityPlayer player, IQuest quest)
	{
		...
		for(int i = 0; i < player.inventory.getSizeInventory(); i++)
		{
			...
			for(int j = 0; j < requiredFluids.size(); j++)
			{
				...
				drain.amount = remaining / stack.stackSize; // Must be a multiple of the stack size
				...
				progress[j] += fluid.amount * stack.stackSize;
                hasDrained = true;
				updated = true;
			}
			
			if(hasDrained && consume) player.inventory.setInventorySlotContents(i, stack);
		}
