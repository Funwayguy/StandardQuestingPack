...
public class EventHandler
{
    ...
    {
        if(event.getEntityPlayer() == null || event.getEntityLiving().worldObj.isRemote || event.isCanceled()) return;
    ...
    {
        if(event.getEntityPlayer() == null || event.getTarget() == null || event.getEntityPlayer().worldObj.isRemote || event.isCanceled()) return;
	...
	{
		if(event.getEntityPlayer() == null || event.getEntityPlayer().worldObj.isRemote) return;
	...
	{
		if(event.getSource() == null || !(event.getSource().getSourceOfDamage() instanceof EntityPlayer) || event.getSource().getSourceOfDamage().worldObj.isRemote || event.isCanceled()) return;
    ...
    {
        if(!(event.getEntity() instanceof EntityPlayer) || event.getEntity().worldObj.isRemote) return;
